  ...
  */
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] pb-32 font-sans selection:bg-slate-900/30">
      ...
      <div className="sticky top-0 z-40 bg-[#F5F5F7]/80 backdrop-blur-2xl border-b border-slate-200/50 pb-4 pt-10 px-5 mb-6 transition-all duration-300">
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="w-12 h-12 bg-slate-900 rounded-[18px] flex items-center justify-center text-white shadow-[0_8px_16px_-6px_rgba(0,0,0,0.3)] transition-transform hover:scale-105 active:scale-95">
                <Truck size={22} strokeWidth={2.5} />
              </div>
              <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-[#F5F5F7] rounded-full flex items-center justify-center">
                <div className={`w-2.5 h-2.5 rounded-full shadow-sm ${(loading || globalLoading) ? 'bg-amber-400 animate-pulse' : 'bg-emerald-500'}`}></div>
              </div>
            </div>
            <div className="flex flex-col justify-center">
              <h1 className="text-[22px] font-black text-slate-900 tracking-tight leading-none mb-1">{operatorName}</h1>
              <p className="text-[12px] font-semibold text-slate-500 tracking-wide flex items-center gap-1.5">
                {(loading || globalLoading) ? 'กำลังซิงค์...' : `อัปเดต: ${lastSyncTime}`}
              </p>
            </div>
          </div>
          <button
            ...
            className="w-11 h-11 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center text-slate-700 shadow-[0_2px_10px_rgba(0,0,0,0.05)] border border-slate-200/80 hover:bg-white active:scale-90 transition-all duration-200"
          >
            <RefreshCw size={18} strokeWidth={2.5} className={(loading || globalLoading) ? 'animate-spin text-amber-500' : ''} />
          </button>
        </div>
      </div>
      ...
      <div className="px-5 mb-6 w-full max-w-4xl mx-auto">
        <div className="bg-[#E5E5EA] p-1.5 rounded-[20px] flex items-center w-full relative shadow-[inset_0_2px_4px_rgba(0,0,0,0.05)]">
          {[
            ...
            return (
              <button
                ...
                className={`flex-1 py-2.5 rounded-[16px] text-[14px] font-black transition-all duration-300 relative flex items-center justify-center gap-2 ${
                  isActive 
                    ? 'bg-indigo-600 text-white shadow-[0_4px_12px_rgba(79,70,229,0.3)] scale-100' 
                    : 'text-slate-500 hover:text-slate-700 hover:bg-slate-300/30 scale-95'
                }`}
              >
                <span>{tab.label}</span>
                {tab.count > 0 && (
                  <span className={`h-5 min-w-[20px] px-1.5 rounded-full flex items-center justify-center text-[10px] font-black transition-colors ${
      ...
      <div className="px-5 space-y-4 max-w-4xl mx-auto w-full">
        ...
        <div className="flex flex-col gap-4 mb-6">
          ...
          <div className="relative w-full group">
            <div className="absolute inset-y-0 left-5 flex items-center pointer-events-none transition-colors group-focus-within:text-indigo-500 text-slate-400">
              <Search size={18} />
            </div>
            <input
              type="text"
              ...
              className="w-full h-14 bg-white/80 backdrop-blur-xl border border-white/60 rounded-[22px] pl-12 pr-5 font-bold text-[14px] text-slate-900 outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all shadow-[0_8px_30px_rgba(0,0,0,0.04)] placeholder:text-slate-400"
            />
          </div>

          {/* Asset Tag Lookup */}
          <div className="bg-white/80 backdrop-blur-xl border border-white/60 rounded-[24px] p-5 shadow-[0_8px_30px_rgba(0,0,0,0.04)]">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center">
                <QrCode size={16} strokeWidth={2.5} />
              </div>
              <p className="text-[14px] font-black text-slate-900 tracking-tight">ค้นหารหัสเครื่อง / Asset</p>
            </div>
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-1 h-12 bg-slate-100/50 border border-slate-200/50 rounded-2xl px-4 text-[13px] font-bold text-slate-900 outline-none focus:bg-white focus:border-indigo-500/50 focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-slate-400"
              />
              <button
                onClick={handleAssetLookup}
                disabled={assetLoading}
                className="h-12 px-6 bg-slate-900 hover:bg-black active:scale-95 text-white rounded-2xl text-[13px] font-black shadow-[0_4px_12px_rgba(15,23,42,0.2)] disabled:opacity-50 disabled:active:scale-100 transition-all flex items-center gap-2"
              >
                {assetLoading ? (
                  <>
            ...
            {assetError && (
              <div className="mt-3 p-3 bg-rose-50/80 border border-rose-100 rounded-xl flex items-center gap-2">
                <p className="text-[12px] font-bold text-rose-600">{assetError}</p>
              </div>
            )}
            {assetResults.length > 0 && (
              <div className="space-y-2 mt-4 max-h-56 overflow-auto scrollbar-hide pr-1">
                {assetResults.map((u: any) => (
                  <div key={u.id || u.assetTag} className="bg-slate-50 border border-slate-100/60 rounded-[18px] p-3.5 flex flex-col gap-1.5 hover:bg-white transition-colors cursor-default">
                    <div className="flex justify-between items-center">
                      <p className="text-[14px] font-black text-slate-900 tracking-tight">{u.assetTag || '-'}</p>
                      <span className="text-[10px] font-black text-slate-600 bg-white shadow-sm border border-slate-100 px-2.5 py-1 rounded-full uppercase tracking-wider">{mapAssetStatusLabel(u.status)}</span>
                    </div>
                    <p className="text-[12px] font-bold text-slate-500 truncate">{u.itemName || '-'} {u.category ? <span className="opacity-70">({u.category})</span> : ''}</p>
                  </div>
                ))}
              </div>
            )}
