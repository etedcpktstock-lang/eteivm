...
export default function JobRequestForm({ items, warehouses = [], customers: initialCustomers, operatorName, onSuccess, onClose, thaiAddressData = [], transactions = [], logisticsJobs = [] }: JobRequestFormProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative">
      ...
      <div className="sticky top-0 z-50 bg-slate-50/80 backdrop-blur-2xl border-b border-slate-200/50 pb-5 pt-8 px-6 mb-2 transition-all duration-300">
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="w-14 h-14 bg-indigo-600 rounded-[22px] flex items-center justify-center text-white shadow-[0_8px_20px_-6px_rgba(79,70,229,0.5)] transition-transform hover:scale-105 active:scale-95">
                <span className="material-symbols-outlined text-[28px] font-bold">assignment_add</span>
              </div>
              <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-slate-50 rounded-full flex items-center justify-center shadow-sm">
                <div className="w-3 h-3 bg-indigo-400 rounded-full animate-pulse"></div>
              </div>
            </div>
            <div className="flex flex-col justify-center">
              <h2 className="text-[22px] font-black text-slate-900 tracking-tight leading-none mb-1.5 uppercase">แจ้งงาน / คืนของ</h2>
              <p className="text-[11px] font-black text-indigo-500 uppercase tracking-[0.25em] leading-none opacity-80">Job Request & Logistics Queue</p>
            </div>
          </div>
          <button 
            onClick={resetAll}
            title="รีเซ็ตฟอร์ม"
            className="btn no-animation w-11 h-11 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center text-slate-400 shadow-[0_2px_10px_rgba(0,0,0,0.05)] border border-slate-200/80 hover:bg-white hover:text-indigo-600 active:scale-90 transition-all duration-200"
          >
